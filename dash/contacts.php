            ...
            <div class="topbar">
                ...
                <div class="main">
                    <div class="search-add-container">
                        <form action="../c-search.php" method="GET" class="d-flex">
                            <input type="text" name="search" placeholder="Search by name..." class="form-control me-2" required>
                            <button type="submit" class="btn btn-primary">Search</button>
                        </form>
                        <a href="../c-create.php" class="btn btn-primary" role="button">Add</a>
                    </div>
                    ...
                    <div class="table-container">
                        <table class="table table-striped table-sm">
                            ...
                            <tbody>
                                <?php
                                // Create a new connection for the 'contact' database
                                $host = "localhost";
                                $user = "root";
                                $pass = "";
                                $contact_db = "contact";
                                $contact_conn = new mysqli($host, $user, $pass, $contact_db);

                                if ($contact_conn->connect_error) {
                                    die("Connection Failed: " . $contact_conn->connect_error);
                                }

                                $sql = "SELECT * FROM user_contact";
                                $result = $contact_conn->query($sql);

                                if ($result === false) {
                                    // Display the SQL error for debugging purposes
                                    echo "<tr><td colspan='6'>Error: " . $contact_conn->error . "</td></tr>";
                                    ...
                                    while ($row = $result->fetch_assoc()) {
                                        echo "<tr>
                                            ...
                                            <td>
                                                <a class='btn btn-primary btn-sm' href='../c-edit.php?id=".htmlspecialchars($row['id'])."'>Update</a>
                                                <a class='btn btn-danger btn-sm' href='../c-delete.php?id=".htmlspecialchars($row['id'])."'>Delete</a>
                                            </td>
                                        </tr>";
                                    }
                                } else {
                                ...
                                // Close the connection to the 'contact' database
                                $contact_conn->close();
                                ?>
                            </tbody>
                        </table>
